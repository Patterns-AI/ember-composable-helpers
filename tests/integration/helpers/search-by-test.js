...
moduleForComponent('search-by', 'Integration | Helper | {{search-by}}', {
  integration: true
});

test('It filters by value', function(assert) {
  this.set('array', emberArray([
    { foo: true, name: 'avocato' },
    { foo: false, name: 'banana' },
    { foo: true, name: 'caramel' }
  ]));

  this.render(hbs`
    {{~#each (search-by 'name' 'AvO' array) as |item|~}}
      {{~item.name~}}
    {{~/each~}}
  `);

  assert.equal(find('*').textContent.trim(), 'avocato', 'caramel and banana are filtered out');
});
...
test('It filters by truthiness', function(assert) {
  ...
  this.render(hbs`
    {{~#each (search-by 'foo' array) as |item|~}}
      {{~item.name~}}
    {{~/each~}}
  `);
...
test('It recomputes the filter if array changes', function(assert) {
  ...
  this.render(hbs`
    {{~#each (search-by 'foo' true array) as |item|~}}
      {{~item.name~}}
    {{~/each~}}
  `);
...
test('It recomputes the filter with a falsy value', function(assert) {
  ...
  this.render(hbs`
    {{~#each (search-by 'foo' false array) as |item|~}}
      {{~item.name~}}
    {{~/each~}}
  `);
...
test('It can be passed an action', function(assert) {
  ...
  this.render(hbs`
    {{~#each (search-by 'foo' (action 'isOdd') array) as |item|~}}
      {{~item.name~}}
    {{~/each~}}
  `);
...
test('It respects objects that implement isEqual interface', function(assert) {
  ...
  this.render(hbs`
    {{~#each (search-by 'foo' firstTarget array) as |item|~}}
      {{~item.name~}}
    {{~/each~}}
  `);
